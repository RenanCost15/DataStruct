...
int* gerar_array_divisiveis(int tamanho, int divisor) {
    int* array = (int*)malloc(tamanho * sizeof(int));
    int contagem = 0;
    
    for (int i = 1; contagem < tamanho; i++) {
        if (i % divisor == 0) {
            array[contagem] = i;
            contagem++;
        }
...
int main(int argc, char **argv) {
    ...
    int* array_divisiveis = gerar_array_divisiveis(n, 1024);
    
    srand(time(NULL));

    for (int i = 0; i < n; i++) {
        inserir(tabela, array_divisiveis[i]);
    }

    int indice_busca = rand() % n;

    clock_gettime(CLOCK_MONOTONIC, &inicio);
    int encontrado = buscar(tabela, array_divisiveis[indice_busca]);
